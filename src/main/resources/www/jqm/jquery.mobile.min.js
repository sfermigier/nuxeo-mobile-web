...
(function(a,e){function c(j){var n=j.charAt(0).toUpperCase()+j.substr(1);j=(j+" "+f.join(n+" ")+n).split(" ");for(var g in j)if(b[g]!==e)return true}a.mobile.media=function(){var j={},n=a("html"),g=a("<div id='jquery-mediatest'>"),i=a("<body>").append(g);return function(m){if(!(m in j)){var o=a("<style type='text/css'>@media "+m+"{#jquery-mediatest{position:absolute;}}</style>");n.prepend(i).prepend(o);j[m]=g.css("position")==="absolute";i.add(o).remove()}return j[m]}}();var d=a("<body>").prependTo("html"),
...
location.protocol+"//"+location.host+location.pathname+"ui-dir/",n=a("<base>",{href:j}).appendTo("head"),g=a("<a href='testurl'></a>").prependTo(d)[0].href;n.remove();return g.indexOf(j)===0}()});d.remove();a.support.boxShadow||a("html").addClass("ui-mobile-nosupport-boxshadow")})(jQuery);
...
(function(a){a.widget("mobile.page",a.mobile.widget,{options:{backBtnText:"Back",addBackBtn:true,degradeInputs:{color:false,date:false,datetime:false,"datetime-local":false,email:false,month:false,number:false,range:"number",search:true,tel:false,time:false,url:false,week:false}},_create:function(){var e=this.element,c=this.options;if(this._trigger("beforeCreate")!==false){e.find("[data-role='page'], [data-role='content']").andSelf().each(function(){a(this).addClass("ui-"+a(this).data("role"))});
e.find("[data-role='nojs']").addClass("ui-nojs");this._enchanceControls();e.find("[data-role]").andSelf().each(function(){var d=a(this),b=d.data("role"),f=d.data("theme");if(b==="header"||b==="footer"){d.addClass("ui-bar-"+(f||d.parent("[data-role=page]").data("theme")||"a"));d.attr("role",b==="header"?"banner":"contentinfo");f=d.children("a");var h=f.hasClass("ui-btn-left"),l=f.hasClass("ui-btn-right");if(!h)h=f.eq(0).not(".ui-btn-right").addClass("ui-btn-left").length;if(!l)l=f.eq(1).addClass("ui-btn-right").length;
if(c.addBackBtn&&b==="header"&&(a.mobile.urlStack.length>1||a(".ui-page").length>1)&&!h&&!d.data("noBackBtn"))a("<a href='#' class='ui-btn-left' data-icon='arrow-l'>"+c.backBtnText+"</a>").click(function(){history.back();return false}).prependTo(d);d.children("h1, h2, h3, h4, h5, h6").addClass("ui-title").attr({tabindex:"0",role:"heading","aria-level":"1"})}else if(b==="content"){f&&d.addClass("ui-body-"+f);d.attr("role","main")}else if(b==="page")d.addClass("ui-body-"+(f||"c"));switch(b){case "header":case "footer":case "page":case "content":d.addClass("ui-"+
b);break;case "collapsible":case "fieldcontain":case "navbar":case "listview":case "dialog":d[b]()}});e.find("[data-role='button'], .ui-bar a, .ui-header a, .ui-footer a").not(".ui-btn").buttonMarkup();e.find("[data-role='controlgroup']").controlgroup();e.find("a:not(.ui-btn):not(.ui-link-inherit)").addClass("ui-link");e.fixHeaderFooter()}},_enchanceControls:function(){var e=this.options;this.element.find("input").each(function(){var c=this.getAttribute("type"),d=e.degradeInputs[c]||"text";e.degradeInputs[c]&&
a(this).replaceWith(a("<div>").html(a(this).clone()).html().replace(/type="([a-zA-Z]+)"/,"type="+d+" data-type='$1'"))});this.element.find("[type='radio'], [type='checkbox']").checkboxradio();this.element.find("button, [type='button'], [type='submit'], [type='reset'], [type='image']").not(".ui-nojs").button();this.element.find("input, textarea").not("[type='radio'], [type='checkbox'], button, [type='button'], [type='submit'], [type='reset'], [type='image']").textinput();this.element.find("input, select").filter("[data-role='slider'], [data-type='range']").slider();
this.element.find("select:not([data-role='slider'])").selectmenu()}})})(jQuery);
